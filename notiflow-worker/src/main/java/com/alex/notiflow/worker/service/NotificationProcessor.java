...
public class NotificationProcessor {
    ...
    public void process(NotificationCreatedEvent event) {
        ...
        if (isTerminal(notification.getStatus())) {
            log.info("Skipping already processed notification notificationId={} status={}",
                    event.notificationId(), notification.getStatus());
            return;
        }
        ...
        while (notification.getAttempts() < maxAttempts) {
            ...
            if (result.sent()) {
                ...
                log.info("Notification sent notificationId={} channel={} attempt={}", event.notificationId(), event.channel(), attempt);
                return;
            }
            ...
            if (result.failureType() == FailureType.PERMANENT) {
                ...
                notificationRepository.save(notification);
                meterRegistry.counter("notiflow.notifications.failed", "channel", event.channel().name(), "type", "permanent").increment();
                log.info("Notification failed permanently notificationId={} reason={}", event.notificationId(), result.reason());
                return;
            }
            ...
            log.info("Notification retry scheduled notificationId={} attempt={} reason={}", event.notificationId(), attempt, result.reason());
        }
        ...
        log.info("Notification dead-lettered notificationId={} attempts={}", event.notificationId(), notification.getAttempts());
    }
